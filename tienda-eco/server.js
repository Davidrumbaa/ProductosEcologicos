...
app.get("/factura/:archivo", async (req, res) => {
  ...
  try {
    ...
    res.send(
      `<pre style="font-family: monospace; background: #f4f4f4; padding: 20px; border-radius: 8px;">${reciboTexto}</pre>`,
    );
  } catch (error) {
    ...
    if (error.code === "ENOENT") {
      ...
      res
        .status(400)
        .send(`<h1>❌ Operación denegada</h1><p>${error.message}</p>`);
    }
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `👉 Entra en tu navegador web y visita: http://localhost:${PORT}/factura/carrito`,
  );
  console.log(
    `👉 O si creaste el segundo archivo: http://localhost:${PORT}/factura/cliente2`,
  );
});
